...
"""
    ...
    2020 Regular season data. The metrics are then logged and the 4 required figures (ROC, Goal Rate, Cumulative Goals and Calibration display)
    are generated.
"""
...
data = pd.read_csv(DATA_PATH + '/clean_test_data_regular.csv', index_col=0)
...
data = pd.read_csv(DATA_PATH + '/clean_test_data_regular_with_optional.csv', index_col=0)
...
model_list = ['LogReg Distance', 'LogReg Angle', 'LogReg Distance + Angle', 'HP Tuned XGBoost', 'AdaBoost 10']
plot_roc_all_feat(data[['is_goal']], y_preds, model_list, '7-1a-ROC', color_list, baseline=False)
plot_goal_rate_all_feat(data[['is_goal']], y_preds, model_list, '7-1b-goal-rate', color_list, baseline=False)
plot_cumulative_rate_all_feat(data[['is_goal']], y_preds, model_list, '7-1c-cumulative', color_list, baseline=False)
plot_calibration_all_feat(data[['is_goal']], y_preds, model_list, '7-1d-calibration', color_list)

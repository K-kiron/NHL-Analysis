...
### <strong>gameSeconds:</strong> int val of number of seconds into the game
### <strong>period:</strong> int val of the period
### <strong>x_coordinate:</strong> float x coordinate of the event
### <strong>y_coordinate:</strong> float y coordinate of the event
### <strong>shotDistance:</strong> distance between the event and the net
### <strong>shotAngle:</strong> counter-clockwise angle between the event and the goal with respect to the x-axis
### <strong>shotType:</strong> type of shot (ex: Wrist Shot, Slap Shot)
### <strong>LastEventType:</strong> Type of the previous event (ex: Shot, Goal)
### <strong>Last_x_coordinate:</strong> float x coordinate of the previous event
### <strong>Last_y_coordinate:</strong> float y coordinate of the previous event
### <strong>timeFromLastEvent:</strong> time interval between the current and previous event
### <strong>DistanceLastEvent:</strong> distance between the current and previous event
### <strong>Rebound:</strong> bool value for if the shot was a rebound or not 
### <strong>changeShotAngle:</strong> if Rebound = True, change in the angle of the puck before and after the rebound
...
We've computed the time (s) since the power-play (PP) started for all teams, the home team, and the away team as a function of period time (s). See the following plots below for the game ID 2016020031 of regular season in 2016 during period 3:
